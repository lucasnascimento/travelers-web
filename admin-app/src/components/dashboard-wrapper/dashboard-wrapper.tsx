...
export const DashboardWrapper = ({
  ...
  React.useEffect(() => {
    const closeSidebarOnOutsideClick = (event: any) => {
      if (isSidebarOpen && !menuRef.current?.contains(event.target as Node)) {
  ...
  return (
    <>
      ...
      <div
        ref={menuRef}
        className={`${isSidebarOpen ? '' : 'hidden'} hs-overlay hs-overlay-open:translate-x-0 transform fixed top-0 start-0 bottom-0 z-[60] w-64 bg-white border-e border-gray-200 pt-7 pb-10 overflow-y-auto lg:block lg:translate-x-0 lg:end-auto lg:bottom-0 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-track]:bg-slate-700 dark:[&::-webkit-scrollbar-thumb]:bg-slate-500 dark:bg-gray-800 dark:border-gray-700`}
      >
        <div className="px-6">
          <p className="flex-none text-xl font-semibold text-gray-800 dark:text-white">
            TravelersApp
